...
/* =========================================================
   ...
   - plan_id
========================================================= */
router.get("/public/month-stats", async (req, res) => {
  const userId = req.query.user_id ? Number(req.query.user_id) : null;

  if (!userId) {
    return res.status(400).json({
      message: "user_id is required for public month stats",
    });
  }
  ...
  try {
    ...
    res.status(500).json({ message: "Public month stats failed", error: e.message });
  }
});
